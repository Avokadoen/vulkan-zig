...
pub fn generateCore(allocator: *Allocator, spec_json: []const u8, writer: anytype) !void {
    ...
    const registry = try std.json.parse(reg.CoreRegistry, &tokens, .{.allocator = &arena.allocator});
...
pub fn generateExtinst(allocator: *Allocator, exstinst_json: []const u8, writer: anytype) !void {
    ...
    const registry = try std.json.parse(reg.ExtensionRegistry, &tokens, .{.allocator = &arena.allocator});
